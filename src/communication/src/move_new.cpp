...
int main(int argc,char ** argv)
{
    ...
    ros::ServiceServer send_Cmd_Round = nh.advertiseService("send_moveRound_cmd", sendCallback);

    ros::Publisher pub_state = nh.advertise<communication::state>("communication/state_move",10);
    ros::Publisher pub_state_Round = nh.advertise<communication::state>("communication/state_moveRound",10);
    ...
    string dev;
    dev=MOVE_ID;
    client.init(UDP_TEST_PORT,UDP_SERVER_IP,rec,dev,"/home/user/log/record/move_new.txt");
