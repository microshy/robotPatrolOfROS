...
bool laserOn = false;
const char* file="/home/robot/log/record/pose.txt";
...
void transOdom(const tf::TransformListener& listener)
{
    ...
    if(laserOn)
    {
        try{
            ...
            //ROS_INFO("MAP:(%f/n,%f/n),ODOM:(%f/n,%f/n)",mapPose.pose.position.x,mapPose.pose.position.y,odomPose.pose.position.x,odomPose.pose.position.y);

            record_time(OsWrite);
            OsWrite<<"[map] ["<<mapPose.pose.position.x<<" ,"<<mapPose.pose.position.y<<"]  ";
            OsWrite<<"[odom] ["<<odomPose.pose.position.x<<" ,"<<odomPose.pose.position.y<<"]"<<endl;
            //更新robot pose
...
void handleOdom(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    if(!laserOn)
    {
        ...
        robotPosePublisher.publish(robotPose);//*
        //ROS_INFO("ODOM:(%f/n,%f/n)",odomX,odomY);
        record_time(OsWrite);
        OsWrite<<"[ODOM] ["<<odomX<<" ,"<<odomY<<"]"<<endl;
    }
}
...
int main(int argc, char** argv)
{
    ...
    ros::Timer timer = nodeHandle.createTimer(ros::Duration(0.02),boost::bind(&transOdom, boost::ref(listener)));
