/*通信模块:与PLC或运动卡通信
  实现功能:1.接受主控的指令,生成指令发送给下位机(PLC或运动卡)
                  2.接收下位机反馈指令,进行校验,并回复给上位机,通知上位机接受正确与否
说明:receFlag:表示接受消息状态 
         connect:表示通信子模块连接状态
         举例说明:若receFlat=false,则需要重新发送该指令. 一是超时,二是数据错误
                          若connnect=false,则需要发送重连指令
*/

#include <communication/udp_client.h>
#include <communication/display.h>
#include <communication/record.h>

/*
 * crc for test  ,when use in normal ,delete the crc=0x10
 */

bool UDP_Client::open()
{
    if((sockfd = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
    {
        perror("Creating socket failed.");
        exit(1);
        return false;
    }
    else
    {
        printf("open ok\n");
        return true;
    }
}

/* 函数名:init
 * 入口参数:端口号,发送数组,接受数组,以及ros节点句柄,设备ID
 * 功能:初始化服务端,绑定端口,初始化数组以及连接状态和数据状态
*/   
void UDP_Client::init(int port,string ip,char *rec,string ID,const char *file)
{
    bzero(&client,sizeof(client));
    client.sin_family=AF_INET;
    client.sin_port=htons(port);
    client.sin_addr.s_addr= inet_addr(ip.c_str());

    order=0x00;
    receFlag=false;
    connect=false;
    dev_ID=ID;
    recBuf=rec;
    logfile = file;
}


/* 函数名:sendInfo
 * 入口参数:指令类型,以及指令数据
 * 功能:生成指令发送给下位机
 */
void UDP_Client::sendInfo(string type ,string data)
{
    socklen_t addrlen;

    string cmdToPlc="";
    char *sendBuf;

    //tranform into the standard command to plc

    unsigned char lenth1;
    unsigned char lenth2;

    unsigned char head=0xFA;
    if(order==255){order=0x00;}else order+=1;
    //order=0;

    int temp=data.size()+1; //zhiling changdu
    if(temp>255)
    {
        lenth1=temp-255;
        lenth2=255;
    }
    else
    {
        lenth1=0;
        lenth2=temp;
    }

    cmdToPlc+=head;cmdToPlc+=order;cmdToPlc+=dev_ID;cmdToPlc+=lenth1;cmdToPlc+=lenth2;

    cmdToPlc+=type;
    cmdToPlc+=data;

    unsigned char crc=getCrc(cmdToPlc);
    unsigned tail=0xFF;
    cmdToPlc+=crc;
    cmdToPlc+=tail;

    sendBuf=(char *)cmdToPlc.c_str();
    int out_len=cmdToPlc.size();
    //调试
    cout<<"send messaage: ";
    HexDump(sendBuf,out_len,0);

    //调试输出到文本
    //打开记录文本
    openFile(logfile,OsWrite);
    OsWrite<<"** send message:";
    string_to_hex(cmdToPlc,OsWrite);
    closeFile(OsWrite);//关闭文本
    
    int sendReal=sendto(sockfd,sendBuf, out_len, 0, (struct sockaddr *)&client, sizeof(client));
    if(sendReal!=out_len) {;}

    receFlag=false;
}

/* 函数名:wait_back
 * 入口参数:最大字节长度,通信时间
 * 功能:等待下位机回复
 */
void UDP_Client::wait_back(int maxsize,int time,int time_ms)
{
    fd_set fds;
    timeval timeout={time,time_ms};
    int net;

    while(!receFlag)
    {
        timeout.tv_sec=time;
        timeout.tv_usec=time_ms;

        FD_ZERO(&fds);
        FD_SET(sockfd,&fds);
        net=0;
        net=select(sockfd+1,&fds,NULL,NULL,&timeout);

        if(net<0)
        {
            exit(-1);
            printf("error\n");
        }
        else if(net==0)
        {
            //打开记录文本
            openFile(logfile,OsWrite);
            OsWrite<<"** receive out";
            OsWrite<<endl;
            closeFile(OsWrite);//关闭文本
            receFlag=false;
            connect=false;     // timeout means connect failed
            break;
        }
        else
        {
            if(FD_ISSET(sockfd,&fds))
            {
                connect=true;
                receive(maxsize);
            }
        }
    }
}

/* 函数名:receive  接受数据
 *入口参数:指令最大字节数
 *功能: 接受数据
*/ 
void UDP_Client::receive(int max)
{
    socklen_t addrlen;
    addrlen=sizeof(client);
    len=0;
    int num=0;
    bzero(recBuf, sizeof(recBuf));
    num =recvfrom(sockfd,recBuf,max,0,(struct sockaddr*)&client,&addrlen);
    len=num;

    //调试
    //cout<<"rece message: ";
    //HexDump(recBuf,len,0);

    char *p=recBuf;

    //调试输出到文本
    //打开记录文本
    openFile(logfile,OsWrite);
    OsWrite<<"** rece message:";
    string log;
    for(int i=0;i<len;i++)
    {
        log+=*(p+i);
    }
    string_to_hex(log,OsWrite);
    OsWrite<<endl;
    closeFile(OsWrite);//关闭文本

    string crc_data="";
    data_in="";
    unsigned char crc_get;
    unsigned char recN=0x00; //数据接受正常与否
    unsigned char getOrder;
    getOrder=*(p+1);
    //cout<<"get order"<<(int)getOrder<<"send order "<<(int)order<<endl;
    
    for (int i=0;i<num-1;i++)
    {
        if(i<num-2)
        {
            crc_data+=*(p+i); //校验数据
        }
        else
        {
            crc_get=*(p+i); //CRC值
        }

        if(5<i&&i<num-2)
        {
            if(i==6)
            {
                recN=*(p+i);
            }
            data_in+=*(p+i); //数据域
        }
    }
    //测试通信
    printf("recN = %X\n",recN);
    printf("crc_get = %X\n",crc_get);
    printf("getCrc = %X\n",getCrc(crc_data));
    printf("getOrder == (int)order : %d, getOrder = %d, (int)order = %d\n",(bool)(getOrder==(int)order), getOrder,(int)order);

    if(crc_get==getCrc(crc_data)&&recN==0x01&&getOrder==(int)order)
        //if(crc_get==getCrc(crc_data)&&recN==0x01)
    {
        receFlag=true;
        //******//
        char *buf;
        buf=(char *)data_in.c_str();
        int out_len=data_in.size();
        len=out_len;
	printf("connect\n");
        //cout<<"message to processor : ";
        //HexDump(buf,out_len,0);
        //****//
    }
    else
        receFlag=false;

    if(num>100)//for battery
    {
        receFlag=true;
    }
}

unsigned char UDP_Client::getCrc(string values)
{
    unsigned char crc_array[256] = {
        ,
    };
    unsigned char crc8 = 0;
    for (int i = 0; i < values.size(); i++)
    {
        crc8 = crc_array[crc8^(unsigned char)(values[i])];
    }
    return crc8;
}

void UDP_Client::close_ser()
{
    close(sockfd);
}
