/*通信模块:与上位机通信
  实现功能:1.接受上位机指令,进行校验回复正常接受与否.
                  2.发布指令话题"communication/cmd"
                  3.订阅状态信息,当上位机查询时,发送状态信息
*/
#include <communication/udp_server.h>
#include <communication/display.h>
#include <communication/state.h>
#include <communication/record.h>

#define  CONNECT_CMD    0x00    //连接指令
#define  ASK_CMD      	0x50  	//查询指令
#define  SET_CMD	0x10	//配置指令
#define  BATTERY_CMD    0xC0    //电源数据

bool UDP_Server::open()
{
    if((sockfd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1)
    {
        perror("Creating socket failed.");
        // exit(1);
        return false;
    }
    else
    {
        printf("open ok\n");
        return true;
    }
}
#define hooost ((in_addr_t)0x6400a8c0)
/* 函数名:init
 * 入口参数:端口号,发送数组,接受数组,以及ros节点句柄
 * 功能:初始化服务端,绑定端口,初始化数组以及订阅器和发布器
 */
void UDP_Server::init(int port,char *send,char *rec,ros::NodeHandle &n,
                      int wrong_time,int broken_time,const char *file)
{
    broken=false;
    connect=false;
    bzero(&server,sizeof(server));
    server.sin_family=AF_INET;
    server.sin_port=htons(port);
    server.sin_addr.s_addr= htonl (INADDR_ANY);
    //server.sin_addr.s_addr= htonl (hooost);
    if(bind(sockfd, (struct sockaddr *)&server, sizeof(server)) == -1)
    {
        perror("Bind()error.");
        // exit(1);
    }
    fcntl(sockfd,F_SETFL,O_NONBLOCK);
    sendBuf=send;
    recBuf=rec;
    //count=0;
    sub_state=n.subscribe("processor/state",10,&UDP_Server::callback,this);
    sub_state2=n.subscribe("/communication/state_battery",2,&UDP_Server::battery_callback,this);
    pub_command=n.advertise<communication::command>("communication/cmd",10);
    time_broken=broken_time;
    time_wrong =wrong_time;
    logfile = file;
    ROS_INFO("state : initializing!");
}

/*函数名:callback
 *功能:回调函数
*/
void UDP_Server::callback(const communication::state &msg)
{
    data_out.clear();
    data_out+=msg.data;
}

void UDP_Server::battery_callback(const communication::state &msg)
{
    data_battery.clear();
    data_battery+=msg.data;
}

/* 函数名:wait_connect  等待连接
 *入口参数:指令最大字节数,以及通信等待时间.
 *功能: 等待连接直至连接成功,成功发布连接成功消息
*/   
void UDP_Server::wait_connect(int maxsize,int time)
{
    fd_set fds;
    timeval timeout={time,0};
    int net;

    while(!connect)
    {
        timeout.tv_sec=time;
        timeout.tv_usec=0;

        ROS_INFO("state : connecting");
        FD_ZERO(&fds);
        FD_SET(sockfd,&fds);
        net=0;
        net=select(sockfd+1,&fds,NULL,NULL,&timeout);
        
        if(net<0)
        {
            printf("exit!");
            exit(-1);
        }
        else if(net==0)
        {
            printf("timeout\n");
        }
        else
        {
            if(FD_ISSET(sockfd,&fds))
            {
                receive(maxsize);
                if(connect)
                {
                    ROS_INFO("construct connect");
                    pub_msg.type=type;
                    pub_msg.command_lenth=cmd_lenth;
                    pub_msg.data.clear();
                    pub_msg.data=data_in;
                    pub_msg.break_flag=false;
                    pub_msg.wrong_flag=!connect;
                    pub_command.publish(pub_msg);
                }
            }
        }
    }
}

/* 函数名:wait_command  等待指令
 * 入口参数:指令最大字节数,以及通信等待时间.
 * 功能: 等待指令,等待时间内到来则正常回复,并发送命令消息，超时则终止,并发布通信中断
 */
void UDP_Server::wait_command(int maxsize)
{
    fd_set fds;
    timeval timeout={time_wrong,0};
    int net;
    connect=true;

    while(1)
    {
        //cout<<endl;
        //ROS_INFO("state : connected");
        timeout.tv_sec=time_wrong;
        timeout.tv_usec=0;

        FD_ZERO(&fds);
        FD_SET(sockfd,&fds);
        net=0;
        net=select(sockfd+1,&fds,NULL,NULL,&timeout);

        if(net<0)
        {
            exit(-1);
        }
        else if(net==0)
        {
            ROS_INFO("%ds  timeout",time_wrong);
            record_string("** rece reconnect timeout:!!!!!!!!!!!!!!!!!!!!",OsWrite);

            pub_msg.wrong_flag=true;
            pub_msg.break_flag=false;
            pub_command.publish(pub_msg);
            connect=false;
            break;
        }
        else
        {
            if(FD_ISSET(sockfd,&fds))
            {
                receive(maxsize);
                pub_msg.type=type;
                pub_msg.command_lenth=cmd_lenth;
                pub_msg.data.clear();
                pub_msg.data=data_in;
                pub_msg.break_flag=false;
                pub_msg.wrong_flag=false;

                pub_command.publish(pub_msg);
            }
        }
        ros::spinOnce();
    }
}

/* 函数名:wait_reconnect  等待重连
 * 入口参数:指令最大字节数,以及通信等待时间.
 * 功能:等待指令,等待时间内到来则正常回复,并发送命令消息,表示重连成功
   超时则终止,并发布通信故障
 */
void UDP_Server::wait_reconnect(int maxsize)
{
    fd_set fds;
    timeval timeout={time_broken,0};
    int net;

    while(!connect)
    {
        //cout<<endl;
        timeout.tv_sec=time_broken;
        timeout.tv_usec=0;
        //ROS_INFO("state : connecting");
        FD_ZERO(&fds);
        FD_SET(sockfd,&fds);
        net=0;
        net=select(sockfd+1,&fds,NULL,NULL,&timeout);

        if(net<0)
        {
            exit(-1);
        }
        else if(net==0)
        {
            ROS_INFO("%ds  timeout",time_broken);
            record_string("** rece reconnect timeout:!!!!!!!!!!!!!!!!!!!!",OsWrite);

            pub_msg.wrong_flag=true;
            pub_msg.break_flag=true;
            pub_command.publish(pub_msg);
            broken=true;
            break;
        }
        else
        {
            if(FD_ISSET(sockfd,&fds))
            {
                receive(maxsize);
                pub_msg.type=type;
                pub_msg.command_lenth=cmd_lenth;
                pub_msg.data.clear();
                pub_msg.data=data_in;
                pub_msg.break_flag=false;
                pub_msg.wrong_flag=!connect;

                pub_command.publish(pub_msg);
            }
        }
    }
}

/* 函数名:receive  接受数据
 * 入口参数:指令最大字节数
 * 功能:接受数据
 */
void UDP_Server::receive(int max)
{
    //count++;
    socklen_t addrlen;
    addrlen=sizeof(server);

    int num=0;
    bzero(recBuf, sizeof(recBuf));

    num =recvfrom(sockfd,recBuf,max,0,(struct sockaddr*)&server,&addrlen);

    if(num>0)
    {
        //调试用 输出到文本
        //打开记录文本
        openFile(logfile,OsWrite);
        OsWrite<<"** rece message:";
        string log;
        char *p=recBuf;
        for(int i=0;i<num;i++)
        {
            log+=*(p+i);
        }
        string_to_hex(log,OsWrite);
        closeFile(OsWrite);
        process(num);

    }
}

/* 函数名:process  数据处理
 * 功能:处理数据,校验上位机数据.产生回复指令
   发送上位机指令给处理器
   若为连接指令.则connect=true;
 */
void UDP_Server::process(int num)
{
    bzero(sendBuf, sizeof(sendBuf));
    int len=num;
    socklen_t addrlen;
    addrlen=sizeof(server);

    if(len>=12)
    {
        char *p=recBuf;
        string out="";

        unsigned char head=*p++;
        unsigned char order=*p++;
        string ID="";
        string data="";
        string data_crc="";

        ID+=*p++;ID+=*p++;ID+=*p++;ID+=*p++;

        // lenth
        unsigned char lenth[]={0,0};
        lenth[0]=*p++;
        lenth[1]=*p++;

        //for processor
        cmd_lenth=0;
        cmd_lenth=(int)lenth[0]*256+(int)lenth[1];

        if(cmd_lenth!=len-10)
        {
            openFile(logfile,OsWrite);
            record_string("** wrong lenth",OsWrite);
            closeFile(OsWrite);
            return;
        }
        unsigned char t=*p++;
        for(int i=0;i<cmd_lenth-1;i++)
        {
            data+=*p++;
        }

        unsigned char check=*p++;
        unsigned char tail=*p;

        unsigned char check_out;//crc
        unsigned char check_in; //crc
        unsigned char rec=0x01; //receive normal   as data

        out+=head;
        out+=order;
        out+=ID;
        string len_out;
        len_out[0]=0x00;
        len_out[1]=0x02;

        data_crc+=head;data_crc+=order;data_crc+=ID;data_crc+=lenth[0];data_crc+=lenth[1];
        data_crc+=t;data_crc+=data;

        if(tail==0xFF)
        {

            check_in=getCrc(data_crc);
            if(check==check_in)
            {
                type="";
                data_in="";
                // for processor
                type+=t;
                data_in=data;

                if(t==ASK_CMD)  // ask for state information
                {

                    len_out[0]=0x00;
                    len_out[1]=0x69;

                    out=out+len_out[0];
                    out=out+len_out[1];
                    out.push_back(t);
                    out+=data_out;
                    check_out=getCrc(out);// update crc
                }
                else if(t==CONNECT_CMD)
                {
                    out=out+len_out[0];
                    out=out+len_out[1];
                    out+=t;
                    out+=rec;
                    check_out=getCrc(out);
                    connect=true;
                }
                else if(t==SET_CMD)
                {
                    out=out+len_out[0];
                    out=out+len_out[1];

                    out+=t;
                    out+=rec;
                    check_out=getCrc(out);
                    time_wrong=data[5];
                    time_broken=data[7]*60; //单位s
                }
                else if(t==BATTERY_CMD)
                {
                    int temp_lenth=data_battery.size()+1;
                    len_out[0]=0x00;
                    len_out[1]=0x85;

                    out=out+len_out[0];
                    out=out+len_out[1];

                    out+=t;
                    //out+=rec;
                    out+=data_battery;
                    check_out=getCrc(out);// update crc
                }
                else
                {
                    out=out+len_out[0];
                    out=out+len_out[1];
                    out+=t;
                    out+=rec;
                    check_out=getCrc(out);
                }
            }
            else// check failed ,return receive error
            {
                out=out+len_out[0];
                out=out+len_out[1];

                out+=t;
                rec=0x00;
                out+=rec;
                check_out=getCrc(out);// update crc
            }
        }
        else //the command not compelete
        {
            out=out+len_out[0];
            out=out+len_out[1];

            out+=t;
            rec=0x00;
            out+=rec;
            check_out=getCrc(out);// default check_out
        }

        out+=check_out;
        out+=tail;

        sendBuf=(char *)out.c_str();
        int out_len=out.size();

        //调试用
        //cout<<"send message :";
        //HexDump(sendBuf,out_len,0);

        //调试输出到文本
        //打开记录文本
        openFile(logfile,OsWrite);
        OsWrite<<"** send messaage:";
        string_to_hex(out,OsWrite);
        OsWrite<<endl;
        closeFile(OsWrite);

        if(connect)
        {
            int a=sendto(sockfd,sendBuf, out_len, 0, (struct sockaddr *)&server, addrlen);
            if(a<=0)
                cout<<"send failed"<<endl;
        }
    }
    else
    {
        printf("send message is wrong\n");
    }
}

/*函数名:getCrc
  功能:产生校验
*/
unsigned char UDP_Server::getCrc(string values)
{
    unsigned char crc_array[256] = {
        ,
    };
    unsigned char crc8 = 0;
    for (int i = 0; i < values.size(); i++)
    {
        crc8 = crc_array[crc8^(unsigned char)(values[i])];
    }
    return crc8;
}

void UDP_Server::close_ser()
{
    close(sockfd);
}
