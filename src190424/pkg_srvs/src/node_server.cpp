...
bool changeMode(pkg_srvs::SrvMode::Request &req,
                pkg_srvs::SrvMode::Response &res)
{
    string cmd = req.cmd;
    res.state = false;
    if(cmd == "MAP_ON")
    ...
    else if(cmd == "MAP_OFF")
    {
        ...
        child = system("rosnode kill /slam_gmapping");
        if(child<0)
        {
            return false;
        }
        ...
        child = system("rosnode kill /laser_node");
        if(child<0)
        {
            return false;
        }
        //child = system("sh ~/.nav/kill_process.sh node_odometry");
        child = system("rosnode kill /node_odometry");
        if(child<0)
        {
            return false;
        }
        //child = system("sh ~/.nav/kill_process.sh node_sensors_tf");
        child = system("rosnode kill /node_sensors_tf");
        if(child<0)
        {
            return false;
        }
    ...
    else if(cmd == "ODOM_OFF")
    {
        ...
        child = system("rosnode kill /node_pose");
        if(child<0)
        {
            return false;
        }
    ...
    else if(cmd == "LASER_OFF")
    {
        ...
        child = system("rosnode kill /amcl");
        if(child<0)
        {
            return false;
        }
        //child = system("sh ~/.nav/kill_process.sh map_server");
        child = system("rosnode kill /map_server");
        if(child<0)
        {
            return false;
        }
        ...
        child = system("rosnode kill /record_pose");
        if(child<0)
        {
            return false;
        }
    ...
    else if(cmd == "LOCATE_OFF")     //激光定位
    {
        int child;
        cmdMsg.cmd = "LOCATE_OFF";
        cmdPublisher.publish(cmdMsg);
        sleep(3);
        child = system("rosnode kill /amcl");
        if(child<0)
    ...
    else if(cmd == "OPEN_LASER")              //  单独开激光
    {
        cmdMsg.cmd = "OPEN_LASER";
        cmdPublisher.publish(cmdMsg);
        res.state = true;
    }
    else if(cmd == "CLOSE_LASER")               //  单独关激光
    {
        int child;
        cmdMsg.cmd = "CLOSE_LASER";
        cmdPublisher.publish(cmdMsg);
        sleep(3);
        child = system("rosnode kill /laser_node");
        if(child<0)
...
bool getLine(pkg_srvs::SrvGetLine::Request &req,
             pkg_srvs::SrvGetLine::Response &res)
{
    double x1, y1, x2, y2;
...
bool getLineTheta(pkg_srvs::SrvGetLineTheta::Request &req,
                  pkg_srvs::SrvGetLineTheta::Response &res)
{
    double x1,y1,x2,y2;
    x1=req.poseA.pose.position.x;
    y1=req.poseA.pose.position.y;
...
bool pointLine(pkg_srvs::SrvPointLine::Request &req,
               pkg_srvs::SrvPointLine::Response &res)
{
    double x,y,a,b,c;
...
bool getYawBias(pkg_srvs::SrvGetYawBias::Request  &req,
                pkg_srvs::SrvGetYawBias::Response &res)
{
...
bool getDistance(pkg_srvs::SrvGetDistance::Request  &req,
                 pkg_srvs::SrvGetDistance::Response &res)
{
    double x1,y1,x2,y2;
    x1=req.poseA.pose.position.x;
    y1=req.poseA.pose.position.y;
